...
export default function ProfilePage() {
    ...
    const [passwordResetForm, setPasswordResetForm] = useState({ otp: '', newPassword: '', confirmPassword: '' });
    ...
    const handleUpdatePassword = async () => {
        ...
        setIsUpdatingPassword(true);
        try {
            const { otp, newPassword: resetPassword, confirmPassword } = passwordResetForm;
            ...
            setPasswordResetForm({ otp: '', newPassword: '', confirmPassword: '' });
    ...
    const handleSendPasswordOtp = async () => {
        ...
        try {
            await forgotPasswordApi({ email: profileData.email.trim() });
            setPasswordOtpSent(true);
            toast.success('Password reset OTP sent to your email');
        } catch (error) {
            toast.error(error.message || 'Unable to send password reset OTP');
        } finally {
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div className="space-y-3">
                                        <SecurityAction
                                            ...
                                            desc={activeRole === 'admin'
                                                ? 'Admin can change password directly with the current password.'
                                                : 'Learner and instructor password changes are verified by email OTP.'}
                                            active={true}
                                        />
                                    </div>
                                                ...
                                                <>
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                        <Input
                                                            ...
                                                            onChange={(event) => setPasswordResetForm((currentState) => ({ ...currentState, otp: event.target.value.replace(/\D/g, '').slice(0, 6) }))}
                                                        ...
                                                        <Input
                                                            ...
                                                            onChange={(event) => setPasswordResetForm((currentState) => ({ ...currentState, newPassword: event.target.value }))}
                                                        />
                                                        <Input
                                                            label="Verify Password"
                                                            type="password"
